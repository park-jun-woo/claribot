...
### Project 실행
```bash
clari plan                      # 모든 Phase 플래닝 절차 시작
clari exec                      # 모든 Phase 실행 시작
clari all                       # plan + exec
```

### Phase 관리
```bash
clari phase create '<json>'  # Phase 등록
clari phase list             # Phase 목록 조회
clari phase <phase-id> plan  # Phase 하위 Task 플래닝 절차 시작
clari phase <phase-id> start # Phase 하위 Task 실행 시작
clari phase <phase-id> all   # plan + start
```
...
### Task 관리
```bash
clari push '<json>'               # Task 추가
clari pop                         # 다음 pending Task (manifest 포함)
clari start <task_id>             # pending → doing
clari complete <task_id> '<json>' # doing → done
clari fail <task_id> '<json>'     # doing → failed
clari status                      # 진행 상황
```
...
```bash
clari memo set '<json>'
clari memo get [phase]:[task]:<key>
clari memo list [phase]:[task]
clari memo del [phase]:[task]:<key>
```
...
# Phase 귀속
clari memo get PH001:api_decisions

# Task 귀속
clari memo get PH001:T042:implementation_notes
```
...
# Phase 메모만
clari memo list PH001

# Task 메모만
clari memo list PH001:T042
```
...
`clari pop` 실행 시 Task + Manifest 함께 반환
...
Claude:
  1. clari project-create '<json>'
  2. clari phase-create '<json>' (여러 번)
  3. clari task push '<json>' (Task 생성)
  4. MASTER_PLAN.md 작성
```
...
Claude:
  while True:
      ...
      # 실행
      clari task start task.id
      ... 작업 ...
      clari task complete task.id '<json>'
...
```bash
# 중요한 발견
clari memo set project:P001:jwt_best_practice '{
  "value": "Use httpOnly cookies for refresh tokens",
  "priority": 1,
...
# Phase별 메모
clari memo set phase:PH002:api_naming '{
  "value": "Use plural nouns for resources",
  "priority": 1
}'

# Task별 메모
clari memo set task:T042:implementation '{
...
**전이**:
- `clari start`: pending → doing
- `clari complete`: doing → done
- `clari fail`: doing → failed
